...
export type MatrixGroup = {
  ...
  color: string;
  rows: string[][];
  rowImages: string[];   // rows ile paralel — her satırın varyant görseli
  pasteBuffer: string;
};
...
export default function MatrixBuilder({
  ...
  const addRow = (id: string) => {
    ...
    updateGroup(id, {
      rows: [...g.rows, Array(value.columns.length).fill("")],
      rowImages: [...g.rowImages, ""],
    });
  };
  ...
  const removeRow = (id: string, ri: number) => {
    const g = value.groups.find((g) => g.id === id)!;
    updateGroup(id, {
      rows: g.rows.filter((_, i) => i !== ri),
      rowImages: g.rowImages.filter((_, i) => i !== ri),
    });
  };
  ...
  const updateRowImage = (id: string, ri: number, url: string) => {
    const g = value.groups.find((g) => g.id === id)!;
    const rowImages = g.rowImages.map((img, i) => (i === ri ? url : img));
    updateGroup(id, { rowImages });
  };

  /* ── Excel yapıştır ── */
  const parsePaste = (id: string) => {
    const g = value.groups.find((g) => g.id === id)!;
    const lines = g.pasteBuffer.trim().split("\n").filter((l) => l.trim());
    ...
    updateGroup(id, {
      rows,
      rowImages: Array(rows.length).fill(""),
      pasteBuffer: "",
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {value.groups.map((group) => (
          <div key={group.id} className="border border-blue-100 rounded-2xl overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-blue-50"
              ...
              <input
                ...
                placeholder="Grup adı (ör: EKO, VİP, LAKLI)"
              />
                ...
                {PRESET_COLORS.map((c) => (
                  <button key={c} type="button"
                    ...
                    className={`w-4 h-4 rounded-full border-2 transition-transform hover:scale-110 ${
                      group.color === c ? "border-gray-600 scale-125" : "border-transparent"
                    }`}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr>
                    ...
                    {/* Görsel sütun başlığı */}
                    <th className="text-left text-[10px] font-bold text-gray-400 uppercase tracking-wider pb-2 pr-2 whitespace-nowrap">
                      <span className="flex items-center gap-1">
                        <ImageIcon size={10} /> Varyant Görseli
                      </span>
                    </th>
                    <th className="w-6" />
                  </tr>
                    ...
                    <tr key={ri}>
                      {value.columns.map((_, ci) => (
                        <td key={ci} className="pr-2 pb-1.5">
                          <input
                            value={row[ci] ?? ""}
                            onChange={(e) => updateCell(group.id, ri, ci, e.target.value)}
                      ...
                      {/* Varyant görsel URL */}
                      <td className="pr-2 pb-1.5 min-w-[160px]">
                        <input
                          value={group.rowImages[ri] ?? ""}
                          onChange={(e) => updateRowImage(group.id, ri, e.target.value)}
                          placeholder="https://... (opsiyonel)"
                          className="w-full border border-gray-100 hover:border-blue-200 focus:border-[#0f75bc] rounded-lg px-2.5 py-1.5 text-xs text-gray-500 focus:outline-none focus:ring-1 focus:ring-[#0f75bc] bg-white transition-colors"
                        />
                      </td>
                      <td className="pb-1.5 align-middle">
                        <button type="button" onClick={() => removeRow(group.id, ri)}
            ...
            <div className="border-t border-blue-50 bg-gray-50 px-4 py-3">
              ...
              <div className="flex gap-2">
                <textarea
                  ...
                  onChange={(e) => updateGroup(group.id, { pasteBuffer: e.target.value })}
                  rows={3}
                  placeholder={`NK\t9x5 cm\t500\t₺420,00\nNKA\t9x5 cm\t1000\t₺620,00`}
                ...
                />
                <button type="button" onClick={() => parsePaste(group.id)}
